...
const BUFF_MAP: Record<string, BuffDetails> = {
  coin_grant_100: {
    title: "+100 Coins Claimed!",
    description: "Coins have been credited to your balance.",
    icon: "🪙",
    color: "text-yellow-400",
  },
  coin_grant_150: {
    title: "+150 Coins Claimed!",
    description: "Coins have been credited to your balance.",
    icon: "🪙",
    color: "text-yellow-400",
  },
  coin_grant_200: {
    title: "+200 Coins Claimed!",
    description: "Coins have been credited to your balance.",
    icon: "🪙",
    color: "text-yellow-400",
  },
  coin_grant_250: {
    title: "+250 Coins Claimed!",
    description: "Coins have been credited to your balance.",
    icon: "🪙",
    color: "text-yellow-400",
  },
  coin_grant_300: {
    title: "+300 Coins Claimed!",
    description: "Coins have been credited to your balance.",
    icon: "🪙",
    color: "text-yellow-400",
  },
  coin_grant_500: {
    title: "+500 Coins Claimed!",
    description: "Mega drop! Coins added to your account.",
    icon: "💎",
    color: "text-blue-400",
  },
  "luck_boost_1.5x": {
    title: "1.5x Luck Active!",
    description:
      "Your mythic pack odds are boosted by 1.5x on your next opening!",
    icon: "🍀",
    color: "text-green-400",
  },
  luck_boost_2x: {
    title: "Double Luck Active!",
    description: "2x Pack Luck active! Open a pack now to use it.",
    icon: "🍀",
    color: "text-green-500",
  },
  luck_boost_3x: {
    title: "3x Mythic Luck Active!",
    description: "Unbelievable luck active! Open a mythic pack now.",
    icon: "🦄",
    color: "text-purple-400",
  },
  discount_10: {
    title: "10% Discount Unlocked!",
    description: "Enjoy 10% off all packs on your next open.",
    icon: "🔥",
    color: "text-red-400",
  },
  discount_15: {
    title: "15% Discount Unlocked!",
    description: "Enjoy 15% off all packs on your next open.",
    icon: "🔥",
    color: "text-red-400",
  },
  discount_20: {
    title: "20% Discount Unlocked!",
    description: "Massive 20% discount active for your next pack!",
    icon: "🔥",
    color: "text-red-500",
  },
  exclusive_pack: {
    title: "Exclusive Pack Unlocked!",
    description: "A special vault pack has been unlocked in your shop!",
    icon: "📦",
    color: "text-indigo-400",
  },
  xp_boost_2x: {
    title: "2x XP Buff Active!",
    description: "Earn double experience progression for your level!",
    icon: "👑",
...
export default function ShopPage() {
  ...
  const [wonItems, setWonItems] = useState<
    { name: string; rarity?: string; value?: number }[]
  >([]);
  ...
  const [permission, setPermission] = useState<
    "granted" | "denied" | "default" | "unsupported" | "ios-needs-pwa"
  >("default");
    ...
    async (buff: string) => {
      ...
      if (buff.startsWith("discount_")) {
        ...
        const val = parseFloat(
          buff.replace("luck_boost_", "").replace("x", ""),
        );
        setActiveLuck(val || 1);
        ...
        try {
          ...
          console.error("Failed to grant coins from buff:", err);
        }
      }
    },
    [fetchUserData],
  ...
  const handleTimerComplete = useCallback(async () => {
    ...
    }).catch((e) => console.error("Ad completion sync failed", e));
  ...
  const loadShopData = useCallback(async () => {
    ...
    try {
      ...
      const [userRes, packRes] = await Promise.all([
        fetch("/api/user/profile", { signal: controller.signal }).catch(() => ({
          ok: false,
          status: 500,
        })),
        fetch("/api/packs", { signal: controller.signal }).catch(() => ({
          ok: false,
          status: 500,
        })),
      ]);
      ...
      if (
        userRes &&
        userRes.ok &&
        typeof (userRes as any).json === "function"
      ) {
        try {
      ...
      if (mappedPacks.length === 0) {
        console.warn(
          "[Shop] No packs found in DB or network error, using fallback sample data",
        );
    ...
    async (ref: string) => {
      ...
      if (
        ...
        try {
          ...
          setErrorDialog({
            message:
              "Failed to claim reward: " + (e.message || "Unknown error"),
          });
        }
      } else if (
        [
  ...
  if (packError)
    return (
      <div className="min-h-screen flex h-[64vh] items-center justify-center bg-[#070707] text-red-400 text-center p-4">
        {packError}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-white p-2 md:p-6 font-sans relative pb-24 overflow-hidden">
      ...
      <div className="mb-4 text-center text-xs text-gray-400">
        Debug: {displayPacks.length} packs to display (hasExclusivePack:{" "}
        {hasExclusivePack ? "true" : "false"})
      </div>
      ...
      <div>
        ...
        <pre className="text-xs bg-black/50 p-2 rounded">
          {JSON.stringify(displayPacks, null, 2)}
        </pre>
      </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6 justify-center items-center">
            {activeDiscount > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full text-[10px] md:text-xs font-bold shadow-[0_0_10px_rgba(239,68,68,0.1)]">
                <span className="animate-pulse">🔥</span> {activeDiscount * 100}
                % Discount
              </div>
            )}
            {activeLuck > 1 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500/10 border border-green-500/20 text-green-400 rounded-full text-[10px] md:text-xs font-bold shadow-[0_0_10px_rgba(34,197,94,0.1)]">
                <span className="animate-pulse">🍀</span> {activeLuck}x Luck
                Boost
              </div>
            )}
            {activeXpBoost && (
        ...
        <div className="relative mb-6 z-20">
          ...
          >
            <span className="font-bold">
              Open {openQuantity} {openQuantity > 1 ? "Packs" : "Pack"}
            </span>
            <ChevronDown
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4 w-full">
          {displayPacks.map((pack, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b ${pack.id === "exclusive_vault_pack" ? "from-indigo-500/10" : "from-white/5"} to-transparent pointer-events-none`}
                />
                ...
                <div className="relative mb-3 mt-2 flex h-[60px] items-center">
                  <div
                    className={`absolute inset-0 blur-xl opacity-40 ${pack.id === "exclusive_vault_pack" ? "bg-indigo-500" : "bg-white"}`}
                  ></div>
                ...
                <h3 className="font-black text-sm md:text-base mb-4 text-center tracking-wide z-10 leading-tight min-h-[40px] flex items-center justify-center">
                  {typeof pack.name === "object" ||
                  typeof pack.name === "function"
                    ? "Unknown Pack"
                    : pack.name}
                </h3>
