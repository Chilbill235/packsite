    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 0.2 }}
          className={`grid w-full gap-2 sm:gap-3 px-2 sm:px-4 grid-cols-${gridCols} ${getMaxHeight()} overflow-y-auto pr-1 sm:pr-2`}
        >
          <AnimatePresence>
            {items.map((item, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative min-w-0 bg-black/40 border ${theme.border}/40 backdrop-blur-xl ${sizeClass} flex flex-col items-center text-center rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden`}
                >
                  <div className={`absolute inset-0 bg-gradient-to-br ${theme.glow} opacity-30 group-hover:opacity-50 transition-opacity duration-500`} />
                  
                  <div className="relative z-10 flex flex-col items-center w-full h-full justify-center">
                    <span className={`${textSizeClass} font-black uppercase tracking-wider ${theme.text} mb-1.5 sm:mb-2 line-clamp-1`}>
