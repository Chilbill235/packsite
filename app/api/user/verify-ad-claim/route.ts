...
import { activeAdSessions } from "../initiate-ad/route";

const REWARD_COOLDOWN_MS = 10 * 60 * 1000;
const MIN_AD_WATCH_TIME_MS = 14500; // ~15 seconds minimum elapsed watch time
...
export async function GET() {
  try {
    ...
    const now = Date.now();
    const timeSinceLastWatch = user?.lastAdWatched
      ? now - user.lastAdWatched.getTime()
      : REWARD_COOLDOWN_MS;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!adToken || typeof adToken !== "string") {
      return NextResponse.json({ error: "Missing ad verification token" }, { status: 400 });
    }

    // 1. Verify token exists & belongs to active session
    const adSession = activeAdSessions.get(adToken);
    if (!adSession || adSession.userId !== session.user.id) {
      return NextResponse.json(
        { error: "Invalid or expired session token. Please start ad again." },
        { status: 403 }
      );
    }
    ...
    // 2. Check minimum elapsed time on ad (~15s)
    const elapsedTime = Date.now() - adSession.startTime;
    if (elapsedTime < MIN_AD_WATCH_TIME_MS) {
      return NextResponse.json(
        { error: "Verification failed. Minimum ad view time not met." },
        { status: 422 }
      );
    }

    // 3. Verify Database Cooldown
    ...
    // Burn token to prevent re-use
    adSession.claimed = true;
    activeAdSessions.delete(adToken);
